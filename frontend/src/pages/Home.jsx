 ...
 *
 * - All users see a welcome message.
 * - Students see RecommendationWidget + AnalyticsDashboard.
 * - Organizations see quick links + AnalyticsDashboard.
 * - Admin users see quick links only.
 */
export default function Home() {
  const { name, email, role } = useContext(AuthContext);
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">
          {role === "student" &&
            "Here are the projects best matched to your profile."}
          {role === "organization" &&
            "Manage your projects and review applications from your dashboard."}
          {role === "admin" &&
      ...
      {/* Student — Recommendations widget */}
      {role === "student" && <RecommendationWidget />}
      ...
      {role === "organization" && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
          ...
          <a  
              key={href}
              href={href}
              className="block p-5 bg-white rounded-xl border border-gray-100 shadow-sm
                         hover:shadow-md hover:border-primaryPale transition text-center
      ...
      {/* Analytics dashboard — student + org only */}
      <AnalyticsDashboard />
    </div>
  );
}